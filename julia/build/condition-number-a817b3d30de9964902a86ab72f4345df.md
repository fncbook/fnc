...
```{math}
  \frac{\quad\dfrac{\norm{\mathbf{h}} }{\norm{\mathbf{x}} }\quad}{\dfrac{\norm{\mathbf{d}} }{\norm{\mathbf{b}}}} 
  = \frac{\norm{\mathbf{h}}\;\norm{\mathbf{b}} }{\norm{\mathbf{d}}\; \norm{\mathbf{x}} }.
```

We can bound $\norm{\mathbf{h}}$ in terms of $\norm{\mathbf{d}}$:
...
Since also $\mathbf{b}=\mathbf{A}\mathbf{x}$ implies $\norm{\mathbf{b}}\le
\norm{\mathbf{A}}\, \norm{\mathbf{x}}$, we derive
...
```{math}
:label: linsyscondb
\frac{\norm{\Delta \mathbf{x}}}{\norm{\mathbf{x}}} \le \kappa(\mathbf{A}) \frac{\norm{\Delta \mathbf{b}}}{\norm{\mathbf{b}}}.
```
...
:::{div}
:width: 100%
```{iframe} https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&entry_id=1_p7vpibn6&config%5Bprovider%5D=%7B%22widgetId%22%3A%221_rtbgb3kv%22%7D&config%5Bplayback%5D=%7B%22startTime%22%3A0%7D
...
```{math}
:numbered: false
\frac{\norm{\mathbf{x}-\tilde{\mathbf{x}}}}{\norm{\mathbf{x}}} \le \kappa(\mathbf{A})
\frac{\norm{\mathbf{h}}}{\norm{\mathbf{b}}}.
```

Here $\tilde{\mathbf{x}}$ is a numerical approximation to the exact solution $\mathbf{x}$, and $\mathbf{h}$ is an unknown perturbation caused by machine roundoff. We will assume that $\norm{\mathbf{h}}/\norm{\mathbf{b}}$ is roughly `eps()`.
