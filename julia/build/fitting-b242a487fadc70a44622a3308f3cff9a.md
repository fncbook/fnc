...
Recalling that $\mathbf{r}^T\mathbf{r}=\norm{\mathbf{r}}_2^2$, and renaming the variables to standardize the statement, we arrive at the general {term}`linear least-squares problem`.
