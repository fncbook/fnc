...
1. $R_{\mathbf{A}}(\mathbf{v})=\lambda,$ the associated eigenvalue.
1. $\nabla R_{\mathbf{A}}(\mathbf{v})=\boldsymbol{0}$.

``` 


If $\mathbf{A}^*=\mathbf{A}$, then the Rayleigh quotient has another interesting property: $ if $\mathbf{v}$ is an eigenvector. By a multidimensional Taylor series, then,
