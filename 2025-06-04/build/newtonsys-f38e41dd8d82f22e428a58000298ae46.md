...
where $\mathbf{J}$ is the Jacobian matrix of $\mathbf{f}$ at $\mathbf{x}$, and $\mathbf{h}$ is a small perturbation from $\mathbf{x}$. The $O(\| \mathbf{h} \|^2)$ term represents the omitted higher-order terms in the Taylor series. 
