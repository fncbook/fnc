...
Therefore, @eq-powereverror now implies that $|\beta_k - \lambda_1|$ is dominated by $|\lambda_2 / \lambda_1|^k$. A more careful statement is

```{math}
:label: poweriterconv
\frac{\beta_{k+1} - \lambda_1}{\beta_{k}-\lambda_1} \rightarrow \frac{\lambda_2}{\lambda_1} \quad \text{as } k\rightarrow \infty.
```
