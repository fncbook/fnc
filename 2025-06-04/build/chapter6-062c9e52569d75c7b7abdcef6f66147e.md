...
```{code-cell}
:tags: [hide-input, remove-output]
plot([], []; layout=(1, 2), xlims=(-1.1, 1.1), ylims=(-1.1, 1.1), show=false, legend=false)
rod_, bob_ = [], []
for k in 1:2
    push!(rod_, plot!([], [], l=4, subplot=k))
    push!(bob_, scatter!([], [], m=(5, :black), subplot=k))
end
anim = @animate for t in range(0, 24, 201)
    θ₁, θ₂, _ = sol(t)
    rod_[1] = ([0, sin(θ₁)], [0, -cos(θ₁)])
    rod_[2] = ([0, sin(θ₂)], [0, -cos(θ₂)])
    bob_[1] = ([sin(θ₁)], [-cos(θ₁)])
    bob_[2] = ([sin(θ₂)], [-cos(θ₂)])
    #plot([0, sin(θ₁)], [0, -cos(θ₁)], l=4;
    #    layout=(1, 2), show=false, legend=false)
    #plot!([0, sin(θ₂)], [0, -cos(θ₂)], l=4;
    #    subplot=2, show=false, legend=false)
