...
⌨ Use {numref}`Function {number} <function-bvp>` to compute the solution to the [Allen–Cahn equation](wiki:Allen-Cahn_equation) in {numref}`Demo %s <demo-nonlinear-allencahn>` with $\epsilon=0.02$. Determine numerically whether it is antisymmetric around the line $x=0.5$---that is, whether $u(1-x)=-u(x)$. You should supply evidence that your answer is independent of $n$. 
``````
