...
A function $g$ is said to satisfy a **Lipschitz condition** with constant $L$ on the interval $S\subset\real$ if, for all $s,t\in S$, 
...
Suppose that $g$ on an interval $S$ satisfies {eq}`lipschitz` with $L<1$, and that $g(x) \in S$ whenever $x\in S$. Then $S$ contains exactly one fixed point $p$ of $g$. If $x_1,x_2,\ldots$ are generated by the fixed-point iteration {eq}`fixedpointiter`, and $x_1 \in S$, then $|x_k-p|\le L^{k-1} |x_1-p|$ for all $k\ge1$.
````
...
✍ In each case, show that the given $g(x)$ has a fixed point at the given $p$ and use {eq}`fpconverge` to show that fixed-point iteration can converge to it.
