...
```{code-cell}
p = np.poly1d(c)    # convert to a polynomial
...
```{code-cell}
V = np.array([ [t[i], 1] for i in range(t.size) ])    # Vandermonde-ish matrix
print(V.shape)
```
...
```{code-cell}
V = np.array([ [t[i]**3,t[i]**2,t[i],1] for i in range(t.size) ])    # Vandermonde-ish matrix
print(V.shape)
```
...
```{code-cell}
V = np.array([ [1, np.log(k+1)] for k in range(100) ])     # fitting matrix
...
print(f"conditioning bound: {kappa * np.finfo(float).eps:.3e}")
```
...
print(f"accurate digits: {-np.log10(relative_err):.2f}")
```
``````
...
```{code-cell}
print(f"norm of (Q^T Q - I) is {norm(Q.T @ Q - np.eye(6)):.3e}")
```
...
```{code-cell}
print(f"norm of (Q_hat^T Q_hat - I) is {norm(Q_hat.T @ Q_hat - np.eye(4)):.3e}")
```
``````
...
print(f"conditioning bound: {cond(A) * np.finfo(float).eps:.3e}")
```
``````
...
P_1 = np.eye(m) - (2 / np.dot(v, v)) * np.outer(v, v)   # reflector
```
...
P_2 = np.eye(m-1) - (2 / np.dot(v, v)) * np.outer(v, v) 
```
