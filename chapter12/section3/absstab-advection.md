...
`````{tab-item} Julia
:sync: julia
````{dropdown} Eigenvalues for advection
:open:
...
`````{tab-item} Julia
:sync: julia
````{dropdown} Eigenvalues for advection–diffusion
:open:
...
`````{tab-item} Julia
:sync: julia
````{dropdown} Eigenvalues for an inflow boundary
:open:
```{include} julia/inflow.ipynb
```
````
`````
