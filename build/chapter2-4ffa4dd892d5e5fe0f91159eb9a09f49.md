...
We can extract the elements on any diagonal using the `diag` function from `numpy.linalg`. The "main" or central diagonal is numbered zero, above and to the right of that is positive, and below and to the left is negative.

```{code-cell} 
from numpy.linalg import diag
print( diag(A) )
```
