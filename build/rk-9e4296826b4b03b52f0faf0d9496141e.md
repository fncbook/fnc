...
``````{exercise}
✍ Use Heun's method to solve the problems in [Exercise 1 above](#problem-rkhandexamples).
``````
...
``````{exercise}
✍ Use RK4 to solve the problems in [Exercise 1 above](#problem-rkhandexamples).
``````
...
``````{exercise}
⌨ Do [Exercise 6.3.4](#problem-SIR), but using {numref}`Function {number} <function-rk4>` instead of `solve`.
``````
