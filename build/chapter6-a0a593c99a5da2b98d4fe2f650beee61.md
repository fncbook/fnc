...
The plot above lacks smoothness because it uses just the discrete values without interpolating between them. It's better to use the dense output to get smooth curves.
...
```{code-cell}
t_ = linspace(0, 80, 1200)
u = vstack([sol.sol(t) for t in t_]).T
fig, ax = subplots()
ax.plot(t_, u[0, :], label="prey")
ax.plot(t_, u[1, :], label="predator")
xlabel("$t$"), ylabel("population")
title("Predator-prey solution")
```

```{code-cell}
t_, u = FNC.euler(predprey, u0, tspan, 800)
u = vstack([sol. for t in t_]).T
ax.scatter(t, u[0, :], label="prey (Euler)", s=1)
ax.scatter(t, u[1, :], label="predator (Euler)", s=2)
ax.legend()
```
