...
```{code-cell}
Q, errest = quad(exp, 0, 1, tol=1e-15, rtol=1e-15)
print(Q)
...
```{code-cell}
Q, errest = quad(lambda x: exp(sin(x)), 0, 1, tol=1e-15, rtol=1e-15)
print(Q)
```
...
```{code-cell}
QI, errest = quad(f, a, b, tol=1e-14, rtol=1e-14)
print(f"Integral = {I:.15f}")
```
...
print(f"Trapezoid estimate is {T:.15f} with error {I - T:.2e}")
```
