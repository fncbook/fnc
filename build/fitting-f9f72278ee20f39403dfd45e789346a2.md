...
```{note}
Another way to describe @linls is as an overdetermined linear system. Because the system $\mathbf{A}\mathbf{x}=\mathbf{b}$, has more rows than columns, it is imposing too many constraints on $\bfx$ to satisfy all of them. 
```
