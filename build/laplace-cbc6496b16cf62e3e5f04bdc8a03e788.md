...
`````{tab-item} Julia
:sync: julia
````{dropdown} Kronecker product
:open:
```{include} julia/kron.ipynb
```
````
`````
...
`````{tab-item} Julia
:sync: julia
````{dropdown} Poisson equation as a linear system
:open:
```{include} julia/fd.ipynb
```
````
`````
...
`````{tab-item} Julia
:sync: julia
````{dropdown} Poisson equation in 2D
:open:
```{include} julia/poisson.ipynb
```
````
`````
