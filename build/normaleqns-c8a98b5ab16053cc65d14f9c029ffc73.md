...
We now solve the general linear least-squares problem in {numref}`Definition {number} <definition-leastsq>`. That is, given $\mathbf{A}\in\real^{m \times n}$ and $\mathbf{b}\in\real^m$, with $m>n$, find the $\mathbf{x}\in\real^n$ that minimizes $\norm{\mathbf{b} - \mathbf{A}\mathbf{x}}_2$.
...
If $\mathbf{x}$ satisfies $\mathbf{A}^T(\mathbf{A}\mathbf{x}-\mathbf{b})=\boldsymbol{0}$, then $\mathbf{x}$ solves the linear least-squares problem, i.e., $\mathbf{x}$ minimizes $\norm{\mathbf{b}-\mathbf{A}\mathbf{x}}_2$.
````
...
```{math}
\begin{split}
    \norm{\mathbf{A}(\mathbf{x}+\mathbf{y})-\mathbf{b}}_2^2 &=
    ...
    &= \norm{\mathbf{A}\mathbf{x}-\mathbf{b}}_2^2 + 2\mathbf{y}^T \mathbf{A}^T(\mathbf{A}\mathbf{x}-\mathbf{b}) + \norm{\mathbf{A}\mathbf{y}}_2^2 \\
...
Given $\mathbf{A}\in \real^{m\times n}$ and $\mathbf{b}\in \real^{m}$, the {term}`normal equations` for the linear least-squares problem $\operatorname{argmin}\norm{\mathbf{b}- \mathbf{A} \mathbf{x}}_2$ are $\mathbf{A}^T(\mathbf{A}\mathbf{x}-\mathbf{b})=\boldsymbol{0}$, or equivalently,
...
```{math}
0 = \mathbf{z}^T\mathbf{A}^T\mathbf{A}\mathbf{z}=(\mathbf{A}\mathbf{z})^T(\mathbf{A}\mathbf{z}) = \norm{\mathbf{A}\mathbf{z}}_2^2,
```
...
Finally, we can repeat the manipulations above to show that for any nonzero $n$-vector $\mathbf{v}$, $\mathbf{v}^T(\mathbf{A}^T\mathbf{A})\mathbf{v}=\norm{\mathbf{A}\mathbf{v}}_2^2\ge 0$, and equality is not possible thanks to the second part of the theorem.
````
...
```{math}
:label: rectcond
\kappa(\mathbf{A}) = \norm{\mathbf{A}}_2 \cdot \norm{\mathbf{A}^{+}}_2.
```

If the rank of $ \mathbf{A}$ is less than $n$ (i.e., if it has linearly dependent columns), then $\kappa(\mathbf{A})=\infty$.
````

Provided that the minimum residual norm $\norm{\mathbf{b}-\mathbf{A}\mathbf{x}}_2$ is relatively small, the conditioning of the linear least-squares problem is close to $\kappa(\mathbf{A})$.
