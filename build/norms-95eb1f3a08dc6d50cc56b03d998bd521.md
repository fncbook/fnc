...
For vectors, we use a **norm** $\norm{\cdot}$, which is a function from $\real^n$ to $\real$ with the following properties for all $n$-vectors $\mathbf{x},\mathbf{y}$ and scalars $\alpha$:[^complexnorm]
...
The last of these properties is known as the **triangle inequality**. It is natural to interpret $\norm{\mathbf{x}}=\norm{\mathbf{x}-\boldsymbol{0}}$ as the distance from $\mathbf{x}$ to the origin and $\norm{\mathbf{x}-\mathbf{y}}$ as the distance from $\mathbf{x}$ to $\mathbf{y}$. We will be using only the three most important vector norms, defined as follows.
...
A vector $\mathbf{x}$ is a {term}`unit vector` if $\norm{\mathbf{x}}=1$.
```

For any nonzero vector $\mathbf{v}$, we can find a unit vector through the normalization $\mathbf{x}=\mathbf{v}/\norm{\mathbf{v}}$. Thus, we can interpret

```{math}
:label: magdir
  \mathbf{v} = \norm{\mathbf{v}} \,\cdot\, \frac{\mathbf{v}}{\norm{\mathbf{v}}}
```
...
\begin{align*}
    \norm{\mathbf{x}}_2 &= \sqrt{ 4 + 9 + 1 + 1 } = \sqrt{15}, \\[1ex]
    \norm{\mathbf{x}}_\infty &= \max\{ 2,3,1,1 \} = 3,\\[1ex]
    \norm{\mathbf{x}}_1 &= 2 + 3 + 1 + 1 = 7.
\end{align*}
...
:::{note}
Most of the time, when just $\norm{\mathbf{x}}$ is written, the 2-norm is implied. However, in this section we use it to mean a generic, unspecified vector norm.
:::
...
```{math}
:label: frobenius
\norm{\mathbf{A}}_F = \left( \sum_{i,j} |A_{ij}|^2 \right)^{1/2}.
```
...
Given a vector norm $\norm{\cdot}_p$, we define an {term}`induced matrix norm` for any $m\times n$ matrix $\mathbf{A}$ as

```{math}
:label: matrixnorm
\norm{\mathbf{A}}_{p} = \max_{\norm{\mathbf{x}}_p=1} \norm{\mathbf{A}\mathbf{x}}_p =
...
```{math}
\norm{\mathbf{A}}_2 = \max_{\norm{\mathbf{x}}_2=1} \norm{\mathbf{A}\mathbf{x}}_2.
```

````{prf:example} Norm of the identity matrix
:label: example-norms-identity

In any induced matrix norm, the identity matrix $\mathbf{I}$ has norm $1$. This is because $\norm{\mathbf{I}\mathbf{x}} = \norm{\mathbf{x}}$ for all vectors $\mathbf{x}$, so the max of $\norm{\mathbf{I}\mathbf{x}}$ over unit vectors is $1$. 
````
...
Let $\norm{\cdot}$ designate a matrix norm and the vector norm that induced it. Then for all matrices and vectors of compatible sizes,

```{math}
:label: normineq1
\norm{\mathbf{A}\mathbf{x}} \le \norm{\mathbf{A}}\cdot \norm{\mathbf{x}}.
```
...
```{math}
:label: normineq3
\norm{\mathbf{A}^k} \le \norm{\mathbf{A}}^k \text{ for any integer $k\ge 0$.}
```
````
...
```{math}
:enumerated: false
\norm{\mathbf{A}\mathbf{B}\mathbf{x}} =\norm{\mathbf{A}(\mathbf{B}\mathbf{x})}\le \norm{\mathbf{A}}\cdot\norm{\mathbf{B}\mathbf{x}} \le
\norm{\mathbf{A}}\cdot\norm{\mathbf{B}}\cdot\norm{\mathbf{x}},
```
...
```{math}
:enumerated: false
\norm{\mathbf{A}\mathbf{B}} = \max_{\mathbf{x}\neq \boldsymbol{0}} \frac{\norm{\mathbf{A}\mathbf{B}\mathbf{x}}}{\norm{\mathbf{x}}} \le
\max_{\mathbf{x}\neq \boldsymbol{0}} \norm{\mathbf{A}}\cdot\norm{\mathbf{B}} = \norm{\mathbf{A}}\cdot\norm{\mathbf{B}}.
```
...
```{math}
:label: mxinfnorm
\norm{\mathbf{A}}_\infty = \max_{1\le \,i \,\le m} \sum_{j=1}^n |A_{ij}|,
```

```{math}
:label: mxonenorm
\norm{\mathbf{A}}_1 = \max_{1\le \,j\, \le n} \sum_{i=1}^m |A_{ij}|.
```
...
✍ **(a)** Draw the unit "circle" in the $\infty$-norm, i.e., the set of all vectors $\mathbf{x}\in\real^2$ such that $\norm{\mathbf{x}}_\infty=1$.
...
✍ Prove using @definition-inducednorm that for any induced matrix norm, matrix $\mathbf{A}$, and scalar $c$, $\norm{c\mathbf{A}} = |c|\cdot \norm{\mathbf{A}}.$
``````
...
**(a)** Find all vectors satisfying $\norm{\mathbf{x}}_\infty=1$ and $\norm{\mathbf{A}\mathbf{x}}_\infty=\norm{\mathbf{A}}_\infty$.

**(b)** Find a vector satisfying $\norm{\mathbf{x}}_1=1$ and $\norm{\mathbf{A}\mathbf{x}}_1=\norm{\mathbf{A}}_1$.

**(c)** Find a vector satisfying $\norm{\mathbf{x}}_2=1$ such that $\norm{\mathbf{A}\mathbf{x}}_2=\norm{\mathbf{A}}_2$. (Hint: Use the definition of $\norm{\mathbf{A}}_2$ as the maximum of $\norm{\mathbf{A}\mathbf{x}}_2$ over unit vectors, and parameterize the unit vectors as $(\cos(\theta),\sin(\theta))$. Then $\twonorm{\mathbf{A}\mathbf{x}}^2$ is a function of $\theta$ that is easy to maximize.)
``````
...
✍ Prove that for any induced matrix norm and nonsingular matrix $\mathbf{A},$ $\norm{\mathbf{A}^{-1}} \ge (\norm{\mathbf{A}})^{-1}.$ (Hint: Apply @theorem-norms-inequalities.)
``````
...
```{math}
\norm{\mathbf{v}}_p \ge \max_{i=1,\ldots,n} |v_i|,
```
...
```{math}
\norm{\mathbf{A}}_p \ge \max_{i,j=1,\ldots,n} |A_{ij}|,
```
...
✍ Prove using @definition-inducednorm that if $\mathbf{D}$ is a diagonal matrix, then $\norm{\mathbf{D}}_2 = \max_{i} |D_{ii}|.$ You may assume the matrix is real and square, but that does not affect the result or the proof in any significant way. (Hint: Let $M=\max_{i} |D_{ii}|.$ Proceed in two stages, showing that $\norm{\mathbf{D}}_2\ge M$ and separately that $\norm{\mathbf{D}}_2\le M.$)
``````

``````{exercise}
:label: problem-norms-neumann

✍ Suppose that $\mathbf{A}$ is ${n\times n}$ and that $\norm{\mathbf{A}}<1$ in some induced matrix norm.

**(a)** Show that $(\mathbf{I}-\mathbf{A})$ is nonsingular. (Hint: Use the definition of an induced matrix norm to show that if $(\mathbf{I}-\mathbf{A})\mathbf{x}=\boldsymbol{0}$ for all nonzero $\mathbf{x}$, then $\norm{\mathbf{A}}\ge 1$.)

**(b)** Show that $\displaystyle \lim_{m\rightarrow \infty} \mathbf{A}^m = \boldsymbol{0}$. (For matrices as with vectors, we say $\mathbf{B}_m \rightarrow \mathbf{L}$ if $\norm{\mathbf{B}_m-\mathbf{L}} \rightarrow 0$.)
