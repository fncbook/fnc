...
````{dropdown} Floating-point arithmetic oddity
:open:
```{include} matlab/arithmetic.ipynb
```
````
`````
...
````{dropdown} Floating-point arithmetic oddity
:open:
```{include} python/arithmetic.ipynb
```
````
`````
``````
