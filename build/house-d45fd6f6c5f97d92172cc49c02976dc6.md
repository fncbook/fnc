...
\mathbf{P}\mathbf{z} =
\begin{bmatrix}
\pm \norm{\mathbf{z}}\\0 \\ \vdots \\ 0
\end{bmatrix} = \pm \norm{\mathbf{z}} \mathbf{e}_1.
```
...
```{math}
:label: hhvector
  \mathbf{w} = \norm{\mathbf{z}} \mathbf{e}_1-\mathbf{z}, \quad \mathbf{v} = \frac{\mathbf{w}}{\norm{\mathbf{w}}}.
```
...
Let $\mathbf{v}$ be defined by {eq}`hhvector` and let $\mathbf{P}$ be given by {eq}`hhreflect`. Then $\mathbf{P}$ is symmetric and orthogonal, and $\mathbf{P}\mathbf{z}=\norm{\mathbf{z}}\mathbf{e}_1$.
````
...
```{math}
\begin{split}
    \mathbf{w}^T\mathbf{w} &= \norm{\mathbf{z}}^2 - 2 \norm{\mathbf{z}} z_1 + \mathbf{z}^T\mathbf{z}
    = 2\norm{\mathbf{z}}(\norm{\mathbf{z}}-z_1),\\
    \mathbf{w}^T\mathbf{z} &= \norm{\mathbf{z}}z_1 - \mathbf{z}^T\mathbf{z} = -\norm{\mathbf{z}}\bigl(\norm{\mathbf{z}}-z_1\bigr),
\end{split}
```
...
\frac{-\norm{\mathbf{z}} \bigl(\norm{\mathbf{z}}-z_1\bigr)}{2\norm{\mathbf{z}} \bigl(\norm{\mathbf{z}}-z_1\bigr)} \mathbf{w}
= \mathbf{z} + \mathbf{w} = \norm{\mathbf{z}}\mathbf{e}_1.
```
````
...
**(b)** ⌨ Given the vector $\mathbf{z}=[1\;2\;3\;4\;5]^T,$ find a sequence of Givens rotations that transforms $\mathbf{z}$ into the vector $\norm{\mathbf{z}}\mathbf{e}_1.$ (Hint: You can operate only on pairs of elements at a time, working from the bottom upwards.)
