...
`````{tab-item} Julia
:sync: julia
````{dropdown} Perimeter of an ellipse
:open:
```{include} julia/ellipse.ipynb
```
````
`````
...
`````{tab-item} Julia
:sync: julia
````{dropdown} Comparing spectral integration methods
:open:
```{include} julia/compare.ipynb
```
````
`````
