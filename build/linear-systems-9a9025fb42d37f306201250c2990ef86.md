...
In fact, when we encounter an expression such as $\bfx = \bfA^{-1} \bfb$ in computing, we interpret it as "solve the linear system $\bfA \bfx = \bfb$" and apply whatever algorithm is most expedient based on what we know about $\bfA$.
