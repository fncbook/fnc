...
where $\alpha_j = \twonorm{\mathbf{y}_j}^{-1}$ is the normalization factor at iteration $j$. Thus, @definition-poweriteration modifies {eq}`powerAkx0` and {eq}`poweriterconverge` only slightly.
