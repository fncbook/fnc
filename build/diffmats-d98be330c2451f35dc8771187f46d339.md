...
```
Here as elsewhere, elements of $\mathbf{D}_x$ that are not shown are zero. We call $\mathbf{D}_x$ a {term}`differentiation matrix`. Each row of $\mathbf{D}_x$ gives the weights of the finite-difference formula being used at one of the nodes.
