...
In order to modularize our codes, we use @function-tensorgrid to define functions and values related to working with tensor-product grids. The function `tensorgrid` creates a grid of points in the rectangle $[a,b]\times[c,d]$ with $m$ and $n$ points in the $x$ and $y$ directions, respectively. It returns the grid points as a tuple of vectors, one for each direction, and the grid spacing as a tuple of scalars. The function `tensorgrid` is used in all of our examples.
