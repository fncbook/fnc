...
Next, suppose that $\mathbf{x}_k = \mathbf{v}_1 + \epsilon \mathbf{w}$, where we have normalized so that $\twonorm{\mathbf{v}_1} = \mathbf{w} = 1$. Since $\mathbf{x}_k$ is approaching a dominant eigenvector, we expect that $\epsilon$ will be small if $k$ is large enough. Then
...
That is, $\beta_k$ estimates $\lamnda_1$ about as well as $\mathbf{x}_k$ estimates $\mathbf{v}_1$.

Note also that

```{math}
:label: eq-powerrq
\beta_k =  =  = R_{\mathbf{A}}(\mathbf{x}_k),
```

where the {term}`Rayleigh quotient` appears because $\twonorm{\mathbf{x}_k}=1$. 
$ is nearly a dominant eigenvector of $\mathbf{A}$, then $\beta_k$ is nearly the dominant eigenvalue $\lambda_1$.
