...
, where $\mathbf{w}$ is a vector of length $mn$. Then the matrix $\mathbf{U}$ has shape $(m+1)\times (n+1)$, and the vector $\mathbf{w}$ has shape $(mn)\times 1$. The vec and unvec operations are defined in such a way that
...
````{tab-item} MATLAB
The function `vec` is built-in to MATLAB, whereas unvec is a particular use case of the `reshape` function.
````

````{tab-item} Python
The function `vec` is built-in to NumPy, whereas unvec is a particular use case of the `reshape` function.
````
