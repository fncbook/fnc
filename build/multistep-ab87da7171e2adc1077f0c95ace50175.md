...
``````{exercise}
:label: problem-multistep-consistency
✍ Show that the unit local truncation error of a multistep method is at least $O(h)$ if and only if the generating polynomials satisfy $\rho(1)=0$ and $\rho'(1) = \sigma(1)$.
``````
