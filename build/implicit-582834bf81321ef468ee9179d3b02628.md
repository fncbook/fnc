...
⌨ Within machine epsilon, the exact solution of the IVP in @demo-implicit-stiff satisfies $\hat{u}(400)=1$. For $n=600,800,1000,\ldots,2000,$ solve that IVP using {numref}`Function {number} <function-ab4>` and {numref}`Function {number} <function-am2>`, keeping track of the error $|u_n-1|$ in each case. Make a log-log convergence plot of the error versus $n$ for each method on the same graph. (They are dramatcally different, for reasons discussed in Chapter 11.)
``````
