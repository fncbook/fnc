...
If the eigenvalues are ordered by distance to $s$, then (asymptotically) one step of inverse iteration reduces the error by the factor $|\lambda_1-s|/|\lambda_2-s|$. As $s \to\lambda_1$, the change in the denominator is negligible. So, if at one point the error $(\lambda_1-s)$ is about $\epsilon$, then the error in the next estimate is reduced by a factor $O(\epsilon)$, making it $O(\epsilon^2)$. Each step now squares the error, which is {term}`quadratic convergence`.
