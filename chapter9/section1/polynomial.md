...
`````{tab-item} Julia
:sync: julia
````{dropdown} Lagrange cardinal polynomials
:open:
```{include} julia/lagrange.ipynb
```
````
`````
...
`````{tab-item} Julia
:sync: julia
````{dropdown} Polynomial interpolation error
:open:
```{include} julia/error.ipynb
```
````
`````
